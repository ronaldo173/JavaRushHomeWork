...
public class Solution {
    ...
    public <T extends Item> T getProxy(Class<T> clas, Class<?>... podClass) {
        Class<?>[] interfaces = new Class[podClass.length + 1];
        System.arraycopy(podClass, 0, interfaces, 0, podClass.length);
        interfaces[interfaces.length - 1] = clas;
        return (T) Proxy.newProxyInstance(clas.getClassLoader(), interfaces, new ItemInvocationHandler());
    }
}
